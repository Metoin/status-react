    ...
    node ('macos') {
      ...
      try {
        ...
        stage('Build (Android) for e2e tests') {
          sh 'cd android && mv app/build/outputs/apk/debug/app-debug.apk app/build/outputs/apk/debug/app-debug.original.apk && ENVFILE=.env.e2e ./gradlew assembleDebug'
        }
        ...
        stage('Upload apk for e2e tests') {
          ...
          withCredentials([string(credentialsId: 'SAUCE_ACCESS_KEY', variable: 'key'), string(credentialsId: 'SAUCE_USERNAME', variable: 'username')]){
            ...
            sh('curl -u ' + username+ ':' + key + ' -X POST -H "Content-Type: application/octet-stream" https://saucelabs.com/rest/v1/storage/' + username + '/' + apk_name + '?overwrite=true --data-binary @android/app/build/outputs/apk/debug/app-debug.apk')
          }
